...
workflow CHANNEL_ALIGN_CREATE_CSV {
    ...
    main:
        ...
        bam_indexed.collectFile(keepHeader: true, skip: 1, sort: true, storeDir: "${outdir}/csv") { meta, bam_file, bai_file ->
            def patient = meta.patient
            def sample  = meta.sample
            def sex     = meta.sex
            def status  = meta.status
            def bam_path   = "${outdir}/preprocessing/mapped/${sample}/${bam_file.name}"
            def bai_path   = "${outdir}/preprocessing/mapped/${sample}/${bai_file.name}"
