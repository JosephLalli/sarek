...
workflow CHANNEL_VARIANT_CALLING_CREATE_CSV {
    ...
    main:
        ...
        vcf_to_annotate.collectFile(keepHeader: true, skip: 1,sort: true, storeDir: "${outdir}/csv"){ meta, _vcf ->
            def patient       = meta.patient
            def sample        = meta.id
            def variantcaller = meta.variantcaller
            def vcf_path = "${outdir}/variant_calling/${variantcaller}/${meta.id}/${_vcf.getName()}"
            ["variantcalled.csv", "patient,sample,variantcaller,vcf\n${patient},${sample},${variantcaller},${vcf_path}\n"]
        }
}
